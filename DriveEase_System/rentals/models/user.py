...
class UserManager(BaseUserManager):
    ...
    def register_user(self, post_data):
        """
        Fat Model Behavior: Handles password hashing via bcrypt and inserts
        the new client record cleanly into the database.
        """
        # 1. Securely hash the password using bcrypt
        hashed_pw = bcrypt.hashpw(post_data['password'].encode(), bcrypt.gensalt()).decode()
        
        # 2. Extract safe phone value
        phone_num = post_data.get('phone', None) if post_data.get('phone', '') != '' else None
        
        # 3. Create and return the user record with 'Client' role
        return self.create(
...
class User(AbstractUser):
    ...
    # Override password length field locally to store longer bcrypt text safely
    password = models.CharField(max_length=255)

    # Attach our custom fat logic manager
    objects = UserManager()
